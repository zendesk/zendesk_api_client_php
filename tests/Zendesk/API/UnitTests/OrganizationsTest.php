...
class OrganizationsTest extends BasicTest {
    ...
    public function testCreateMany() {
        // Test for CreateMany Method - requires an Organization Name as parameter
        $this->mock->expects($this->any())
                    ->method('createMany')
                    ->with($this->isType('string'), $this->isType('string'))
                    ->will($this->returnCallback(function($first_name, $second_name) { $this->organizations->organizations[0]->name = $first_name; 
                                                                                        $this->organizations->organizations[1]->name = $second_name; 
                                                                                        return $this->organizations; }));

        $organizations = $this->mock->createMany('Organization I', 'Organization II');
        $this->assertEquals(is_object($organizations), true, 'Should return an object');
        $this->assertEquals(is_array($organizations->organizations), true, 'Should return an object containing an array called "organizations"');
        $this->assertGreaterThan(0, $organizations->organizations[0]->id, 'Returns a non-numeric id for first organization');
        $this->assertEquals('Organization I', $organizations->organizations[0]->name, 'Name of first test organization does not match');
        $this->assertGreaterThan(0, $organizations->organizations[1]->id, 'Returns a non-numeric id for second organization');
        $this->assertEquals('Organization II', $organizations->organizations[1]->name, 'Name of second test organization does not match');
    }
