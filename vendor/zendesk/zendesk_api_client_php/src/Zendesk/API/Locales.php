...
class Locales extends ClientAbstract {
     ...
     * List all locales
     */
     // allows parameter to determine if current user 'current.json' method
     // allow parameter to determine if all locales for account 'locales.json' method
     // or parameter for individual agent 'agent.json'
    ...
    /*
     * Detect the locale of the current user
     */
     
    public function currentLocale() {
	    $endPoint = Http::prepare('locales/current.json');
	    $response = Http::send($this->client, $endPoint);
	    if ((!is_object($response)) || ($this->client->getDebug()->lastResponseCode != 200)) {
            throw new ResponseException(__METHOD__);
        }
