...
// Users statuses may be missing  --------  GET /api/v2/users/statuses.json - will be removed from documenation - not public
// Show many users may be missing   -----  GET /api/v2/users/show_many.json?ids={ids}
...
class Users extends ClientAbstract {
    ...
    */
    ...
    /*
	     ...
	     * endpoint works in terminal, not here.
	     * need to modify to exclude id (I guess you could include if wanted, but that is not how end user is identified
	     * need to pass parameter email and password
     */
    ...
    /*
     * Suspend a user
     * Added option to set 'suspended' as false
     */
    ...
    /*
     * Search for users
     * Added search by external id to users.php
     */
     ...
     */
     // Removed '@'. from $endPoint, array('user[photo][uploaded_data]' => '@'.$params['file']), 
     // because PHP version > 5.5.0
    public function updateProfileImage(array $params) {
        ...
        error_log("line 242 - ". $params['file']);
        print_r($response);
        if ((!is_object($response)) || ($this->client->getDebug()->lastResponseCode != 200)) {
            print_r($response);
            throw new ResponseException(__METHOD__);
     ...
     * Changed response to include success message
     */
    ...
    public function changePassword(array $params) {
        ...
        if ($this->client->getDebug()->lastResponseCode != 200) {
            //print_r("Request failed, please try again");
            //throw new ResponseException(__METHOD__);
        }
        $this->client->setSideload(null);
        return $response;
