...
*/
    public static function send($client, $endPoint, $json = null, $method = 'GET', $contentType = 'application/json') {
        ...
        if($method == 'POST') {
            ...
            if(is_array($json) && (isset($json['uploaded_data']))) {
                curl_setopt($curl, CURLOPT_SAFE_UPLOAD, 1);
				$args['uploaded_data'] = new CURLFile($json['uploaded_data']);
				curl_setopt($curl, CURLOPT_POSTFIELDS, $args);
				$http_header = array("Expect: 100-continue", 'Content-Type: multipart/form-data');
			}
        } else
        if($method == 'PUT') {
            $curl = curl_init($url);
            ...
            curl_setopt($curl, CURLOPT_POSTFIELDS, $json);
             if(is_array($json) && (isset($json['user[photo][uploaded_data]']))) {
             	error_log("61 - ".$json['user[photo][uploaded_data]']);
                curl_setopt($curl, CURLOPT_SAFE_UPLOAD, 1);
